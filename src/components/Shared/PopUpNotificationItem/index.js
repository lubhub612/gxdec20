...
import { PopUpNotificationItemContainer } from "./styles";

const PopUpNotificationItem = (props) => {
  const { t } = useTranslation();
  const { notificationList, color } = props;
  React.useEffect(() => {
  ...
  return (
    <PopUpNotificationItemContainer>
      <div className="popup-title">{t("Notifications")}</div>
      {notificationList.length === 0 && (
        <div className="notification-empty">
          <div className="notification-empty-icon">
            <NotificationEmptyIcon color={props.color} />
          </div>
