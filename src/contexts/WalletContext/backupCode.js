...
export const WalletProvider = (props) => {
  ...
  const initWallet = async () => {
    if (window.web3 instanceof Web3) return;
  ...
  const connectWalletChain = useCallback(
    async (chainId, chainName, nativeCurrency, rpcUrls, blockExplorerUrls) => {
      let ethereum = window.ethereum;
      if (ethereum === undefined) return;

      console.log("5 logs", chainId);
      try {
  ...
  const connectWallet = async (w) => {
    ...
    if (
      defWallet._web3ReactContext.library === undefined ||
      defWallet.chainId !== parseInt(walletConfig.chainId, 16)
    ) {
